...
async def scrape_epoca_cosmeticos(url):
    async with async_playwright() as playwright:
        ...
        products = await page.query_selector_all(
            'div[data-testid="productItemComponent"]'
        )
        lojas = []
        ...
        for idx, product in enumerate(products):
            try:
                ...
                preco_el = await product.query_selector(
                    '.product-price_spotPrice__k_4YC'
                )
                if not preco_el:
                    preco_el = await product.query_selector(
                        '.product-price_priceList__uepac'
                    )
                ...
                preco_final = (
                    float(preco_final_str) if preco_final_str else 0.0
                )
                ...
                desc_el = await detail_page.query_selector(
                    'p[data-product-title="true"]'
                )
                if desc_el:
                    descricao = await desc_el.inner_text()
                    descricao = descricao.strip()
                else:
                    meta_desc = await detail_page.query_selector(
                ...
                loja_el = await detail_page.query_selector(
                    '.pdp-buybox-seller_sellerInfo__BmOa4 a span'
                )
                if loja_el:
...
async def extract_data_from_amazon(target_url: str) -> list:
    ...
    async with async_playwright() as p:
        browser = await p.chromium.launch(
            headless=True
        )  # Mantido como False para depuração
        try:
        ...
        try:
            ...
            try:
                desc_element = await page.locator(
                    '#productTitle'
            ...
            try:
                imagem = await page.locator(
                    '#landingImage'
                ).first.get_attribute('src', timeout=10000)
            except Exception as e:
            ...
            try:
                ...
                review_span = page.locator(
                    'a.a-popover-trigger.a-declarative span.a-size-base.a-color-base[aria-hidden="true"]'
                ).first
                ...
                if review_text and re.match(
                    r'^\d+\.\d$', review_text.replace(',', '.')
                ):
                    review = float(review_text.replace(',', '.'))
                else:
                    print(
                        'Review não encontrada ou inválida, tentando fallback.'
                    )
                    # Tenta o seletor de fallback
                    review_span_fallback = page.locator(
                        'span.a-size-base.a-color-base'
                    ).first
                    review_text_fallback = (
                        await review_span_fallback.inner_text(timeout=10000)
                        or ''
                    )
                    review_text_fallback = (
                        review_text_fallback.strip()
                    )  # Remove espaços em branco
                    print(
                        f"Texto da review (fallback): '{review_text_fallback}'"
                    )
                    if review_text_fallback and re.match(
                        r'^\d+\.\d$', review_text_fallback.replace(',', '.')
                    ):
                        review = float(review_text_fallback.replace(',', '.'))
                    else:
                        print(
                            'Review não encontrada no fallback, usando padrão 4.5'
                        )
            except Exception as e:
            ...
            try:
                ...
                try:
                    ...
                    seller_name = (
                        await seller.inner_text(timeout=10000)
                        or 'Não informado'
                    )
                    seller_name = re.sub(
                        r'Vendido por\s*', '', seller_name
                    ).strip()

                    price_span = page.locator(
                        'div.a-section.a-spacing-micro span.a-offscreen'
                    ).first
                    price_text_raw = (
                        await price_span.inner_text(timeout=10000) or '0.00'
                    )
                    price_text = re.sub(
                        r'[^\d,.]', '', price_text_raw
                    ).replace(',', '.')
                    if re.match(r'^\d+\.\d+$', price_text):
                        ...
                        print(
                            f'Preço inválido na página principal: {price_text}'
                        )
                    ...
                    if seller_name != 'Não informado' and preco_final > 0.0:
                        ...
                        key_sku = (
                            f'{key_loja}_{sku}'
                            if sku != 'SKU não encontrado'
                            else f'{key_loja}_sem_sku'
                        )
                        lojas.append(
                            {
                                'sku': sku,
                        ...
                        print(
                            f'Vendedor principal capturado: {seller_name}, Preço: {preco_final}'
                        )
                except Exception as e:
                    print(
                        f'Erro ao extrair vendedor/preço da página principal: {e}'
                    )
            except Exception as e:
            ...
            try:
                compare_button = page.get_by_role(
                    'button',
                    name=re.compile(
                        'Comparar outras.*ofertas|Ver todas as ofertas'
                    ),
                )
                ...
                details_link = page.get_by_role(
                    'link', name='Ver mais detalhes sobre esta'
                )
            ...
            await page.wait_for_timeout(
                5000
            )  # Aumentado para garantir carregamento dinâmico
            ...
            try:
                ...
                for i, offer in enumerate(offer_elements, 1):
                    try:
                        ...
                        preco_final = 0.0
                        try:
                            price_span = offer.locator(
                                'span.aok-offscreen'
                            ).first
                            price_text_raw = (
                                await price_span.inner_text(timeout=10000)
                                or '0.00'
                            )
                            price_text = re.sub(
                                r'[^\d,.]', '', price_text_raw
                            ...
                            if re.match(r'^\d+\.\d+$', price_text):
                                ...
                                print(
                                    f'Preço inválido na oferta {i}: {price_text}'
                                )
                        except Exception:
                            try:
                                price_whole = (
                                ...
                                if re.match(r'^\d+\.\d+$', price_text):
                                    ...
                                    print(
                                        f'Preço inválido na oferta {i} (fallback): {price_text}'
                                    )
                            except Exception as e:
                                print(
                                    f'Erro ao extrair preço na oferta {i}: {e}'
                                )

                        # Extrai vendedor
                        seller_name = 'Não informado'
                        try:
                            seller = offer.locator(
                            ...
                            print(
                                f'Erro ao extrair vendedor na oferta {i}: {e}'
                            )

                        # Evita duplicatas
                        if any(s['loja'] == seller_name for s in lojas):
                            print(
                                f'Vendedor {seller_name} já capturado, ignorando duplicata'
                            )
                            continue
                        ...
                        print(
                            f'Oferta {i} capturada: {seller_name}, Preço: {preco_final}'
                        )
                    except Exception as e:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if 'mercadolivre' in url.lower():
                ...
                lojas = await scrape_epoca_cosmeticos(
                    url
                )  # Usa await em vez de asyncio.run
...
async def process_urls(urls):
    ...
    print(
        f'Total de URLs a processar: {total_urls} (incluindo {len(sem_dado)} URLs de execuções anteriores)'
    )
